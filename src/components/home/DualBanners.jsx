  ...
  const inner = (
    <motion.div initial={{ opacity: 0, scale: 0.97 }} animate={{ opacity: 1, scale: 1 }}
      ...
      <div className="aspect-square sm:aspect-[4/3] w-full bg-muted">
        {section.image_url
          ? <img src={cmsImageSrc(section.image_url, 'large')} srcSet={cmsImageSrcSet(section.image_url)} sizes="(max-width: 640px) 50vw, 620px" alt={title} width={800} height={600} loading="lazy" decoding="async" onError={handleImageError} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
